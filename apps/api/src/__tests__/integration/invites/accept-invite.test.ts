...
describe('POST /invites/accept', () => {
  ...
  it('should not be able to accept an invite with invalid code', async () => {
    await makeUser()
    const token = await authenticate(app)

    const response = await supertest(app.server)
      .post('/invites/accept?code=invalidcode')
      .set('Authorization', `Bearer ${token}`)

    expect(response.status).toBe(400)
  })

  it('should not be able to accept an invite belonging to another user', async () => {
    const owner = await makeUser()
    const org = await makeOrganization(owner.id)
  ...
  it('should not be able to accept an expired invite', async () => {
    const owner = await makeUser()
    const org = await makeOrganization(owner.id)
    const member = await makeUser({ email: 'member@example.com' })
  ...
  it('should not be able to accept an invite without authentication', async () => {
    const response = await supertest(app.server).post(
      '/invites/accept?code=anycode',
    )
