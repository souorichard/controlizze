...
describe('PUT /orgs/:slug/categories/:categoryId', () => {
  ...
  it('should be able to update a category', async () => {
    ...
    const response = await supertest(app.server)
      .put(`/orgs/${org.slug}/categories/${category.id}`)
      .set('Authorization', `Bearer ${token}`)
      .send({ name: 'Updated', color: '#0000ff', type: 'INCOME' })

    expect(response.status).toBe(204)
  })

  it('should not be able to update a category without authentication', async () => {
    const user = await makeUser()
    const org = await makeOrganization(user.id)
    const category = await makeCategory(user.id, org.id)
  ...
  it('should not be able to update a category as MEMBER', async () => {
