...
import { updateOrganizationAvatar } from '@/http/organization/update-organization-avatar'
...
import { uploadOrganizationAvatar } from '@/http/storage/upload-organization-avatar'
import { ActionResponse } from '@/interfaces/action-response'

import { OrganizationAvatarFormData } from './_components/organization-avatar-form'
...
export async function updateOrganizationAvatarAction({
  file,
}: OrganizationAvatarFormData): Promise<ActionResponse> {
  const currentOrganization = await getCurrentOrganization()
  ...
  try {
    ...
    await updateOrganizationAvatar({
      organization: currentOrganization!,
      avatarUrl: url,
    })
    ...
    return {
      success: true,
      message: 'Successfully updated organization avatar.',
      savedUrl: url,
    }
  } catch (error) {
    if (error instanceof HTTPError) {
